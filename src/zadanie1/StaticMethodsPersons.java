...
public class StaticMethodsPersons <T> {
    ...
    public static void showPeople() {
        ...
        animals.add(new Dog("Pimpek", 12,Sex.MALE,people.getHuman(7)));
        animals.add(new Dog("Billy", 8,Sex.MALE,people.getHuman(1)));
        animals.add(new Horse("Dragon", 13,Sex.CASTRATED,people.getHuman(5)));
        animals.add(new Cat("Kicia", 2,Sex.FEMALE,people.getHuman(3)));
        animals.add(new Cat("Odyn", 16,Sex.MALE,people.getHuman(3)));
        animals.add(new Dog("Didi", 1,Sex.FEMALE,people.getHuman(2)));



        int animalCounter;
        for(Human el: people.getPeople()){
            System.out.println(el.toString());
            animalCounter = 0;
            for(Animal aEl: animals.getAnimals()){
                if(el.equals(aEl.getOwner())){
                    System.out.println(aEl.toString());
                    animalCounter++;
                }
            }
            if(animalCounter==0){
                System.out.println("(!) - nie ma zwierząt");
            }
        }
